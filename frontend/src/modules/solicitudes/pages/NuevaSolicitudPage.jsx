...
import { ChevronRight, ChevronLeft, Save, Bookmark, Sparkles, Trash2 } from 'lucide-react';

const DRAFT_STORAGE_KEY = 'siresa_solicitud_draft';
...
export default function NuevaSolicitudPage({ onSaveSuccess }) {
  ...
  // Estado para borrador guardado en localStorage
  ...
  const saveDraftToStorage = useCallback((overrideData) => {
    if (step > 0 && step < 6) {
      const payload = {
        step,
        moduloTipo,
  ...
  }, [step, moduloTipo, formData, documentosCargados]);

  // Guardar automáticamente cuando cambien los datos durante la captura
  useEffect(() => {
    if (step > 0 && step < 6) {
      saveDraftToStorage();
    }
  }, [step, moduloTipo, formData, documentosCargados, saveDraftToStorage]);

  // Recuperar Borrador
  const handleRestoreDraft = () => {
    if (!savedDraft) return;
    setModuloTipo(savedDraft.moduloTipo);
    if (savedDraft.formData) setFormData(savedDraft.formData);
    if (savedDraft.documentosCargados) setDocumentosCargados(savedDraft.documentosCargados);
    setStep(savedDraft.step);
    setSavedDraft(null);
    toast.success(`Borrador de ${formatModulo(savedDraft.moduloTipo)} recuperado con éxito.`);
  };
  ...
  const handleManualSaveDraft = () => {
    saveDraftToStorage();
    toast.success('💾 Solicitud guardada como borrador local. Podrás continuarla más tarde.');
  };

  // ── Protección contra Cierre Involuntario de Pestaña (beforeunload) ───────
  useEffect(() => {
  ...
  }, [step, saveDraftToStorage]);

  // ── Escuchar eventos de guardado/descarte disparados por el Layout ────────
  useEffect(() => {
    const onSaveBorrador = () => saveDraftToStorage();
    const onDiscardBorrador = () => handleDiscardDraft();

    window.addEventListener('sdr-solicitud-guardar-borrador', onSaveBorrador);
    window.addEventListener('sdr-solicitud-descartar-borrador', onDiscardBorrador);

    return () => {
      window.removeEventListener('sdr-solicitud-guardar-borrador', onSaveBorrador);
      window.removeEventListener('sdr-solicitud-descartar-borrador', onDiscardBorrador);
    };
  }, [saveDraftToStorage]);
  ...
  if (step === 0) {
    return (
      <div className="space-y-6 animate-fadeIn">
        ...
        <SeleccionModulo handleSelectModulo={handleSelectModulo} />
      </div>
    );
  }
